...
void Play_state::enter(GLuint program, GLuint* a, GLuint* b, GLint s) {
	...
	for (int i = 0; i < MAX_EFFECT; ++i) {
		effectinfos[i].alpha_num = 0.0;
		effectinfos[i].color[0] = 0.0;
		effectinfos[i].color[1] = 0.0;
		effectinfos[i].color[2] = 0.0;
		effectinfos[i].Trans_line = glm::mat4(1.0f);
		effectinfos[i].Scale_time = glm::mat4(1.0f);
		effectinfos[i].Rotate_line = glm::mat4(1.0f);
		effectinfos[i].Ison = GL_FALSE;
	}
...
void Play_state::draw() {
	...
	for (int i = 0; i < MAX_EFFECT; ++i) {
		if (effectinfos[i].Ison) {
			TR = glm::mat4(1.0f);
			TR = effectinfos[i].Rotate_line * effectinfos[i].Trans_line * TR_default * effectinfos[i].Scale_time;
			glUniformMatrix4fv(modelLocation, 1, GL_FALSE, &TR[0][0]);

			glUniform3f(objColorLocation, effectinfos[i].color[0], effectinfos[i].color[1], effectinfos[i].color[2]);

			glUniform1f(alpha, effectinfos[i].alpha_num);
	...
	for (int i = draw_notenum; i < trigger_notenum + 1; ++i) {
		if (noteinfos[i].Trigger && !noteinfos[i].Processed) {
			if ((noteinfos[i].noteline == key) && (noteinfos[i].playline == selected_num)) {
				add_effect(noteinfos[i].noteline, noteinfos[i].playline);
...
void Play_state::add_effect(GLint n, GLint p) {
	for (int i = 0; i < MAX_EFFECT; ++i) {
		if (!effectinfos[i].Ison) {
			effectinfos[i].turn_on(n, p);
			break;
		}
	}
}

void Play_state::update_effect() {
	for (int i = 0; i < MAX_EFFECT; ++i) {
		if (effectinfos[i].Ison) {
			effectinfos[i].update_(Get_Game_Framework().get_frame_time());
		}
	}
}