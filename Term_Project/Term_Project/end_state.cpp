...
End_state& Get_End_state() { return end_state; }

void End_state::enter(GLuint program, GLuint* a, GLuint* b) {
...
void End_state::handle_events(Event evnt) {
	switch (evnt.type) {
	case KEYBOARD:
	{
		switch (evnt.key) {
...
void End_state::GenBuffer() {
	glGenVertexArrays(3, vao);
	glGenBuffers(3, vbo);
}

void End_state::InitBuffer() {
	// play line //
	glBindVertexArray(vao[0]);
